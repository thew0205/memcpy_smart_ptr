...
TEST(MEMCPY_UNIQUE_PRR, Memcpy_SEND_function)
{
    ...
    bool copied = ptr1.memcpy_send(buffer, [](void *dest, const memcpy_unique_ptr<int> *src)
...
TEST(MEMCPY_UNIQUE_PRR, Memcpy_receive_function)
{
    ...
    bool sent = ptr1.memcpy_send(buffer, [](void *dest, const memcpy_unique_ptr<int> *src)
                                 { return memcpy(dest, src, sizeof(memcpy_unique_ptr<int>)); });
    CHECK(sent);
    CHECK(ptr1.get() == nullptr);

    memcpy_unique_ptr<int> ptr2;
    bool received = ptr2.memcpy_receive(buffer, [](memcpy_unique_ptr<int> *dest, const void *src)
                                        { return memcpy(dest, src, sizeof(memcpy_unique_ptr<int>)); });
    CHECK(received);
    CHECK(*ptr2 == 70);
}
